...
def build_grading_prompt(category, question, model_answer, student_answer, evaluation_criteria, query_status=None, error_message=None):
    ...
    if category == "SQL":
        ...
        prompt = f"""
                    ...
                    1. 각 평가기준(정확도, 가독성, 효율성 등)에 대해 **5점 단위(0, 5, 10, ..., 100)로만** 부분 점수를 각각 매겨주세요. (예: 정확도 50점, 가독성 20점, 효율성 15점)
                    2. 각 기준별 점수의 합이 100점이 되도록 최종 점수도 함께 출력해 주세요.
                    3. 각 평가기준별로 '참고할 점'을 한두 문장으로 구체적으로 작성해 주세요. (예: 잘한 점, 부족한 점, 개선점 등)
                    4. 
                    5. 마지막 피드백은 평가자가 참고할 수 있도록, 학생에게 직접 전달하는 친근한 말투가 아니라 틀린 부분, 부족한 점, 개선점 등 객관적이고 구체적인 정보 위주로 작성해 주세요.
        ...
        prompt = f"""
                ...
                - 평가기준: {evaluation_criteria}
                {grading_criteria_str}
                학생의 답안이 모범답안과 다르더라도, 논리적 오류가 없고 실행결과가 같다면 높은 점수를 주세요.
                아래 조건을 꼭 지켜서 평가해 주세요.

                1. 각 기준별 점수의 합이 100점이 되도록 최종 점수도 함께 출력해 주세요.
                3. 라이브러리나 패키지 사용 여부는 평가하지 않습니다.
                4. 마지막 피드백은 평가자가 참고할 수 있도록, 학생에게 직접 전달하는 친근한 말투가 아니라 틀린 부분, 부족한 점, 개선점 등 객관적이고 구체적인 정보 위주로 작성해 주세요.

                결과는 아래 형식으로 출력해 주세요.

                [최종 점수] 80
                [피드백] 전반적으로 함수 구조와 내장 함수 활용이 적절하나, 소수점 자리 오차가 있음
                """        
